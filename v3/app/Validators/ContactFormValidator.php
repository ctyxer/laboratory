...
class ContactFormValidator extends FormValidator
{
    private static function validation(Request $request)
    {
        return FormValidator::make($request->all(), [
            ...
            'Возраст' => 'required',
            'Пол' => 'required',
            'Номер_телефона' => 'required|regex:/^([0-9\s\-\+\(\)]*)$/|min:9',
            'Электронная_почта' => 'required|email:rfc,dns',
            'День_рождения' => 'required|date',
            'Сообщение' => 'required'
        ], [
    ...
    {
        $validation = ContactFormValidator::validation($request);
